...
const ASCII_TAG_STOPWORDS = new Set(["A", "An", "And", "But", "For", "From", "How", "Or", "The", "To", "With"])
const TAG_KEYWORDS = [
	"Agent",
	"RAG",
	"Skill",
...
function countKeywordMatches(text, keywords) {
	...
	const tokens = new Set(tokenizeForMatch(normalized))
...
function detectCategory(title, markdown) {
	...
	const categoryScores = Object.entries(CATEGORY_RULES).map(([category, keywords]) => ({
		category,
		score:
			countKeywordMatches(title, keywords) * 10 +
			countKeywordMatches(importantContent, keywords) * 4 +
			countKeywordMatches(markdown, keywords),
	}))

	const ranked = categoryScores.sort((left, right) => right.score - left.score)
	const bestCategory = ranked[0]?.category
	const bestScore = ranked[0]?.score ?? 0
	const secondScore = ranked[1]?.score ?? 0
	const fallback = summarizeFallbackCategory(title, markdown)
...
function titleCaseAsciiTag(token) {
	return token
		.toLowerCase()
		.split(/([.+-])/)
		.map((part) => {
...
function normalizeTagCandidate(candidate) {
	const value = candidate.trim()
	if (!value) {
		return ""
	}

	const asciiKey = value.toLowerCase()
	if (TAG_ALIASES.has(asciiKey)) {
		return TAG_ALIASES.get(asciiKey) ?? ""
	}

	if (/^[a-z0-9][a-z0-9.+-]{1,19}$/i.test(value)) {
		return titleCaseAsciiTag(value)
	}

	return value
}

function collectTagSignals(text, weight, scores) {
	for (const keyword of TAG_KEYWORDS) {
		const hits = countKeywordMatches(text, [keyword])
		if (hits > 0) {
			const normalized = normalizeTagCandidate(keyword)
			scores.set(normalized, (scores.get(normalized) ?? 0) + hits * weight)
		}
	}

	const asciiMatches = text.match(/\b[A-Za-z][A-Za-z0-9.+-]{1,19}\b/g) ?? []
	for (const match of asciiMatches) {
		const normalized = normalizeTagCandidate(match)
		if (!normalized || ASCII_TAG_STOPWORDS.has(normalized)) {
			continue
		}

		if (!TAG_ALIASES.has(normalized.toLowerCase())) {
			continue
		}
...
function extractCandidateTags(title, markdown) {
	...
	collectTagSignals(title, 4, scores)
	collectTagSignals(importantContent, 2, scores)

	return [...scores.entries()]
		.filter(([tag, score]) => tag && score >= 2)
